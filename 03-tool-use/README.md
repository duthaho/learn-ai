...
```
User Message ────▶ Agentic Loop Entry Point
                         │
                    ┌─────▼──────┐
...
```
03-tool-use/
├── index.html          # Visual architecture guide (open in browser)
├── tools.py            # Tool definitions (JSON schemas) + implementations
└── app.py              # Standalone demo — run with: python app.py
```
...
#### File 2: `app.py` — Standalone Demo

The demo script runs 8 scenarios sequentially, exercising the full agentic loop from a single Python file. Run it with `python app.py`.

##### Demo 1: Tool Definitions Inspection

Prints the exact tool definitions sent to the model. Use this to:
...
##### Demo 2: Single Tool Call (Weather Query)

Asks the model about the weather in Tokyo.

**What to observe:**
- The agentic loop runs 2 iterations: iteration 1 is the tool call, iteration 2 is the final response
- The tool call log shows exactly what the model requested and what it received
- The model's response incorporates the tool result naturally into conversation

##### Demo 3: Order Lookup

Asks the model to look up orders for a customer email address.

**What to observe:**
- The model decides which tool to call based on the question
- It extracts the email from the user's message and passes it as a tool argument
- The response incorporates the structured order data into natural language

##### Demo 4: Calculation (LLMs Can't Do Math)

Asks the model to calculate a math expression.

**What to observe:**
- Instead of guessing the answer, the model delegates to the `calculate` tool
- This demonstrates why tool use matters: LLMs are unreliable at arithmetic, but a tool gives exact results
- The calculator uses a security whitelist to prevent code injection via `eval()`

##### Demo 5: Multi-Tool Parallel Calls

Asks the model about weather in multiple cities and a calculation in one query.

**What to observe:**
- The model emits multiple `tool_use` blocks in a single response (parallel calls)
- All tool calls are executed and all results sent back in one user message
- This is a performance optimization: one round trip instead of multiple sequential iterations

##### Demo 6: No Tool Needed

Sends a simple greeting that doesn't require any tool.

**What to observe:**
- The model responds directly without calling any tools
- `stop_reason` is `"end_turn"` immediately — zero tool iterations
- The agentic loop exits on the first iteration

##### Demo 7: Force Tool (`tool_choice` Parameter)

Forces the model to call a specific tool even when the query doesn't explicitly require it.

**What to observe:**
- Even though the question doesn't explicitly ask for weather, the model is forced to call `get_weather`
- The model still generates sensible arguments (e.g., city: "Paris")
...
Runs a multi-turn conversation where the model retains context from previous turns, including previous tool calls.

**What to observe:**
- In the second turn, the model remembers data from the first turn (it's in the message history)
- The model only calls tools for new information — it doesn't re-fetch data it already has
- If you strip tool_use/tool_result messages from history, the model loses context and will re-call tools

**Production note:** In a real system, you'd store the full message history (including tool_use and tool_result messages) in a database.
